...
export default function CallSheet() { const { id } = useLocalSearchParams<{ id: string }>(); const isDark = useColorScheme() === "dark";
  return (<View style={[styles.container, { backgroundColor: isDark ? "#1a1a1a" : "#F5F5F5" }]}><Text style={[styles.title, { color: isDark ? "#fff" : "#1c4a22" }]}>Call</Text><Text style={styles.placeholder}>Call nursery #{id}</Text></View>);
}
